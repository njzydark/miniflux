...
func RefreshFeed(store *storage.Storage, userID, feedID int64, force bool) error {
	...
	if originalFeed.IgnoreHTTPCache || response.IsModified(originalFeed.EtagHeader, originalFeed.LastModifiedHeader) {
		...
		processor.ProcessFeedEntries(store, originalFeed, user, force)
		...
		isShouldUpdateExistingEntries := force || !originalFeed.Crawler
		if storeErr := store.RefreshFeedEntries(originalFeed.UserID, originalFeed.ID, originalFeed.Entries, isShouldUpdateExistingEntries); storeErr != nil {
			originalFeed.WithError(storeErr.Error())
			store.UpdateFeedError(originalFeed)
			return storeErr
		}
